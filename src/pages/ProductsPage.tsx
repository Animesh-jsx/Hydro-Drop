...
const fallbackProducts = [
  ...
  },
  {
    ...
    name: 'Hydra Max',
    tags: ['All-Day', 'Sports'],
    description: 'Stay hydrated all day long. Perfect for sports, gym sessions, and outdoor activities in Kolkata.',
    price: 55,
    oldPrice: 65,
    volume: '1L',
  ...
  return (
    <PageTransition>
      <HeroBanner
        ...
        subtitle="Experience the purity of nature in every drop. Sourced from pristine Himalayan aquifers and enriched with essential minerals for your vitality."
      />
              ...
              {products.map((p, i) => (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-sm text-gray-500 mb-4 line-clamp-2">{p.description || p.desc}</p>
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-primary font-bold text-lg">₹{p.price}</span>
                        {p.oldPrice > 0 && <span className="text-gray-400 text-sm line-through ml-2">₹{p.oldPrice}</span>}
                      </div>
                      <button
                        onClick={() => addItem({
                          id: p._id || p.id,
      ...
      <section className="py-20 bg-linear-to-r from-emerald-900 to-primary-darkest text-white">
        <AnimatedSection className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-emerald-200 mb-8 max-w-xl mx-auto">
            We offer special pricing for corporate events, offices, and regular home delivery subscriptions across Kolkata and West Bengal.
          </p>
